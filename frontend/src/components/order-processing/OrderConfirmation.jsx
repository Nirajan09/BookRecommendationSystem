...
export default function OrderConfirmation() {
  ...
  // Helper to build full address display
  const fullAddress = `${order.street}${order.ward ? ", " + order.ward : ""}, ${order.city}, ${order.province}${order.postal_code ? ", " + order.postal_code : ""}`;
      ...
      <div className="bg-white shadow-md rounded-lg p-6 mt-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-gray-700 text-sm">
          ...
          <div>
            ...
            <p>{paymentMethodNames[order.payment_method] || order.payment_method}</p>
          </div>
          <div>
          ...
          <div>
            ...
            <p>Rs. {Number(order.shipping_cost).toFixed(2)}</p>
          </div>
        </div>
      ...
      <div className="bg-white shadow-md rounded-lg p-6 mt-6">
        ...
        <div className="mt-2 text-gray-600">
          ...
          <span className="ml-1 font-medium">
            {/* You may replace with dynamic dates if available */}
            3-5 business days
          </span>
        </div>
      </div>
