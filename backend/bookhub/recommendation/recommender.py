...
def get_user_based_recommendations(user_id, top_n=8):
    ...
    # Return empty list if user not found or no ratings
    ...
    similar_users = [(idx, score) for idx, score in enumerate(similarity_scores)
                     if idx != user_idx and score > 0]
...
def get_item_based_recommendations(user_id, top_n=8):
    ...
    rated_book_indices = [idx for idx, rating in enumerate(user_ratings) if rating > 0]

    predictions = []
    for book_idx in range(sparse_matrix.shape[1]):
        if user_ratings[book_idx] > 0:
            continue  # Already rated by user

        sims = []
        for rated_idx in rated_book_indices:
            sim = cosine_similarity(
                sparse_matrix[:, book_idx].reshape(1, -1),
                sparse_matrix[:, rated_idx].reshape(1, -1)
            )[0][0]
            sims.append((user_ratings[rated_idx], sim))
        numerator = sum(r * s for r, s in sims)
        denominator = sum(abs(s) for _, s in sims)
        if denominator > 0:
            predicted = numerator / denominator
            predictions.append((book_idx, predicted))

    predictions.sort(key=lambda x: x[1], reverse=True)
    reverse_book_map = {idx: isbn for isbn, idx in book_map.items()}
    top_books_isbn = [reverse_book_map[idx] for idx, _ in predictions[:top_n]]
